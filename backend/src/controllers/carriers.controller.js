...
async function list(req, res) {
  ...
  if (search) {
    params.push(`%${search}%`);
    conditions.push(`(name ILIKE $${params.length} OR mc_number ILIKE $${params.length})`);
  }
  if (status) {
    params.push(status);
...
async function create(req, res) {
  const { name, mc_number, phone, email, address, status } = req.body;
  const { rows } = await query(
    `INSERT INTO carriers (name, mc_number, phone, email, address, status)
     VALUES ($1, $2, $3, $4, $5, COALESCE($6, 'Approved'))
     RETURNING *`,
    [name, mc_number || null, phone || null, email || null, address || null, status || null]
  );
...
async function update(req, res) {
  const { name, mc_number, phone, email, address, status } = req.body;
  const { rows } = await query(
    `UPDATE carriers SET name = $1, mc_number = $2, phone = $3, email = $4, address = $5, status = $6
     WHERE id = $7 RETURNING *`,
    [name, mc_number || null, phone || null, email || null, address || null, status || 'Approved', req.params.id]
  );
  if (!rows[0]) throw new ApiError(404, 'Carrier not found.');
  res.json({ carrier: rows[0] });
}
